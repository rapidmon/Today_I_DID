...
// MARK: - 아무 동작 없는 인텐트 (탭 흡수용)

struct NoOpIntent: AppIntent {
    static var title: LocalizedStringResource = "없음"
    static var description: IntentDescription = "아무 동작 없음"
...
struct TetrisProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<TetrisEntry>) -> Void) {
        // 리셋 요청 확인 (앱에서 ExtensionStorage를 통해 설정)
        if let defaults = UserDefaults(suiteName: TetrisGameEngine.suiteName) {
            let resetVal = defaults.integer(forKey: "resetRequested")
            if resetVal == 1 {
                TetrisGameEngine.resetGame()
                defaults.set(0, forKey: "resetRequested")
            }
        }

        // 자동 스폰 (큐에 블록이 있고 활성 블록이 없으면)
...
struct TetrisWidgetView: View {
    ...
    private var headerBar: some View {
        HStack(spacing: 4) {
            ...
            if entry.state.gameOver {
                ...
                // GAME OVER가 아닐 때 NEW는 NoOp (앱 열림 방지)
    ...
    private var sidePanel: some View {
        VStack(spacing: 4) {
            // NEXT 박스 — 앱 열기 링크
            Link(destination: URL(string: "todayidid://")!) {
                VStack(spacing: 4) {
                    Text("NEXT")
                        .font(NeonColors.pixelFont(16))
